...
Compilation also requires the Halide 12.0.0 distribution, which is not checked into this repo. Download it manually:
1. Go to the [Halide v12.0.0 releases page](https://github.com/halide/Halide/releases/tag/v12.0.0) and download the prebuilt archives.
2. Extract each archive's contents (its `bin/`, `include/`, `lib/`, `share/` folders) directly into the matching platform dir below — the gn build (`BUILD.gn`) reads these paths, so the layout must match exactly:
```
extern/Halide/
├── linux/{bin,include,lib,share}
├── osx/{bin,include,lib,share}
└── windows/
    ├── x64/{bin,include,lib,share}     # bin, lib each have a Release/ subfolder
    └── Win32/{bin,include,lib,share}   # only needed for 32-bit builds; same layout
```
